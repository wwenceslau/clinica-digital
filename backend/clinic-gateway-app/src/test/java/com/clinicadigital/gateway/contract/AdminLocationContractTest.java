...
/**
 * T158 [Phase 18] Contract tests for admin locations endpoints.
 *
 * Refs: FR-018, FR-019, FR-020
 */
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@Testcontainers
class AdminLocationContractTest {
    ...
    private static final String FHIR_PRACTITIONER_IDENTIFIER =
            "[{\"system\":\"https://saude.gov.br/sid/cpf\",\"value\":\"00000000088\"}]";
    private static final String FHIR_PRACTITIONER_NAME =
            "[{\"use\":\"official\",\"text\":\"Admin T158\"}]";
    ...
    static final PostgreSQLContainer<?> POSTGRES =
            new PostgreSQLContainer<>("postgres:15-alpine")
                    .withDatabaseName("clinica_admin_location_contract_test")
    ...
    @Test
    void adminCreatesLocationReturns201() {
        UUID tenantId = insertTenant("t158-create", "Tenant T158 Create");
        UUID orgId = insertOrganization(tenantId, "T158001", "Org T158");
        UUID sessionId = insertAdminSession(tenantId, orgId, "admin-create@t158.local", 10);
        ...
        assertThat(response.getBody()).contains("Unidade Centro");
    }

    @Test
    void adminListsLocationsReturns200() {
        UUID tenantId = insertTenant("t158-list", "Tenant T158 List");
        UUID orgId = insertOrganization(tenantId, "T158002", "Org T158 List");
        UUID sessionId = insertAdminSession(tenantId, orgId, "admin-list@t158.local", 10);
        insertLocation(tenantId, orgId, "Unidade Sul", true);
        ...
        assertThat(response.getBody()).contains("Unidade Sul");
    }

    @Test
    void adminDeactivatesLocationReturns200AndInactive() {
        UUID tenantId = insertTenant("t158-deactivate", "Tenant T158 Deactivate");
        UUID orgId = insertOrganization(tenantId, "T158003", "Org T158 Deactivate");
        UUID sessionId = insertAdminSession(tenantId, orgId, "admin-deactivate@t158.local", 10);
        UUID locationId = insertLocation(tenantId, orgId, "Unidade Norte", true);
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).contains("\"accountActive\":false");
    }

    @Test
    void profile20CannotCreateLocationReturns403() {
        UUID tenantId = insertTenant("t158-forbidden", "Tenant T158 Forbidden");
        UUID orgId = insertOrganization(tenantId, "T158004", "Org T158 Forbidden");
        UUID sessionId = insertAdminSession(tenantId, orgId, "user@t158.local", 20);

        String requestBody = """
                {
                  "organizationId": "%s",
                  "displayName": "Unidade Bloqueada"
                }
                """.formatted(orgId);
    ...
    private UUID insertAdminSession(UUID tenantId, UUID orgId, String email, int profile) {
        ...
        String pwHash = passwordService.hashPassword("S3nha@T158");
